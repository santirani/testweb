...
else if (agent.indexOf('https://pib.gov.in/SubscribeRelease/Firefox/') > -1) {
    browser.agent = browser.Firefox;
    browser.version = parseFloat(agent.match(/ Firefox\/(\d+\.\d+)/)[1]);
    browser.name = 'Firefox';
    browser.hasDebuggerStatement = true;
}
else if (agent.indexOf('https://pib.gov.in/SubscribeRelease/AppleWebKit/') > -1) {
    browser.agent = browser.Safari;
    browser.version = parseFloat(agent.match(/ AppleWebKit\/(\d+(\.\d+)?)/)[1]);
    browser.name = 'Safari';
}
else if (agent.indexOf('https://pib.gov.in/SubscribeRelease/Opera/') > -1) {
...
$type._resolveUrl = function WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('https://pib.gov.in/') + 1);

    if (!url || !url.length) {
        return baseUrl;
    }

    if (url.charAt(0) === 'https://pib.gov.in/') {
        var slashslash = baseUrl.indexOf('://');
        if (slashslash === -1) {
            throw Error.argument("baseUrl", Sys.Res.badBaseUrl1);
        }

        var nextSlash = baseUrl.indexOf('https://pib.gov.in/', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('https://pib.gov.in/');
        if (lastSlash === -1) {
